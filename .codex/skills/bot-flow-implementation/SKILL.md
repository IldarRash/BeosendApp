...
1. Read the feature brief and the UX scenario it maps to (`docs/product/feature-roadmap.md` links the
   spec sections). Identify the screens, buttons, and the 2вЂ“3 tap happy path.
2. **ApiClient** вЂ” add typed methods in `apps/bot/src/api-client.ts` for the endpoints you need; parse
   responses with the matching `packages/types` contract.
3. **Keyboards/messages** вЂ” build `InlineKeyboard`s with namespaced callback-data constants (extend
   the `menu.ts` pattern). Keep payloads to IDs.
4. **Handlers** вЂ” `bot.command` / `bot.callbackQuery` / conversation steps that: read the update,
