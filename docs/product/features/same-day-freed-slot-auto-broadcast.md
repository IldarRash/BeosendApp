...
Automatically notify a manager-selected Telegram audience when an explicit user booking cancellation leaves usable capacity in a public group training later that same Europe/Belgrade calendar day. The automation is disabled until a manager configures and enables it, and the existing manual `freed-up` broadcast is unchanged.

The smallest end-to-end slice is: persist one global policy, handle the successful `POST /bookings/:id/cancel` path, honor existing waitlist priority, create one event with dedicated delivery records, perform one event-level eligibility recheck, and make at most one intentional Telegram send attempt per selected recipient.
...
- Add a dedicated freed-slot event table containing at least the cancellation/booking identity, training occurrence identity, creation time, event status, and enough immutable context to audit why the event was selected. Enforce one event for the qualifying cancellation action.
- Add a dedicated freed-slot delivery table containing the event identity, recipient identity, send-attempt state, and Telegram outcome metadata. Enforce one delivery record per `(event_id, recipient_id)`.
...
- Only a successful explicit `POST /bookings/:id/cancel` request may create this automatic event.
...
- Individual-session cancellation, manager decline, booking transfer, capacity edits, schedule edits, and any other path that happens to increase capacity do not trigger this automation.
...
- **Explicit trigger only.** The automation is entered only after a successful `POST /bookings/:id/cancel`; incidental capacity changes and all excluded cancellation/decline/transfer paths are ignored.
...
- **Usable capacity.** After the cancellation commits, and again at the single event-level pre-send recheck, the occurrence must remain bookable under the backend's authoritative capacity rules.
...
- **At-most-one intentional attempt.** Each selected recipient is claimed through the dedicated delivery record and receives at most one intentional Telegram send call. Failed or outcome-ambiguous attempts are recorded and are not retried. No claim of exactly-once external delivery is made because a provider/network ambiguity can leave actual delivery unknown.
...
- A successful explicit `POST /bookings/:id/cancel` for a public visible group training later on the same `Europe/Belgrade` date can create one freed-slot event after commit.
- No event is created for an individual session, a non-public group, a manager decline, a transfer, a capacity/schedule change, or any cancellation path other than the explicit booking-cancel endpoint.
...
- If it passes, each claimed delivery produces at most one intentional Telegram send call; successful, failed, and ambiguous outcomes are persisted without retry.
- Reinvoking internal handling for the same qualifying cancellation does not create a second event or a second intentional delivery attempt.
...
- Successful explicit booking cancellation, public visible group, same `Europe/Belgrade` date, before start, free capacity, and no blocking waitlist: create one event after commit.
...
- Manager decline, transfer, capacity edit, schedule edit, and non-endpoint cancellation paths: do not trigger.
...
- Re-enter handling for the same cancellation and concurrently claim the same delivery: database uniqueness/atomic state allows one event and at most one intentional send call per recipient.
- Telegram success, definite failure, timeout, and outcome-ambiguous failure are recorded; none is retried.
...
- **Which cancellation triggers it?** Decision: only a successful explicit `POST /bookings/:id/cancel`.
...
- **How are failures handled?** Decision: persist the outcome and do not retry or sweep. External exactly-once delivery is not promised.
