...
import type { AutoAssignResult, Training } from "@beosand/types";
import type {
  CourtLoadGridView,
...
const COURT_WORKING_HOURS_KEY = ["court-working-hours"] as const;
 ...
 */
export function useCourtLoad(date: string | null): UseQueryResult<CourtLoadGridView, Error> {
...
export function useCourtWorkingHours(
  year: number,
  month: number,
  enabled = true
): UseQueryResult<CourtWorkingHoursMonthView, Error> {
  const api = useApiClient();
  return useQuery({
    queryKey: [...COURT_WORKING_HOURS_KEY, year, month],
    queryFn: () => api.courtWorkingHours(year, month),
    enabled
  });
}

export function useSaveCourtWorkingHoursMonth(): UseMutationResult<
  CourtWorkingHoursMonth,
  Error,
  { year: number; month: number; openTime: string; closeTime: string }
> {
  const api = useApiClient();
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input) => api.upsertCourtWorkingHoursMonth(input),
    onSuccess: () =>
      Promise.all([
        queryClient.invalidateQueries({ queryKey: COURT_WORKING_HOURS_KEY }),
        queryClient.invalidateQueries({ queryKey: COURT_LOAD_KEY })
      ]).then(() => undefined)
  });
}

export function useSaveCourtWorkingHoursDay(): UseMutationResult<
  CourtWorkingHoursDayOverride,
  Error,
...
> {
  ...
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input) => api.upsertCourtWorkingHoursDay(input),
    onSuccess: () =>
      Promise.all([
        queryClient.invalidateQueries({ queryKey: COURT_WORKING_HOURS_KEY }),
        queryClient.invalidateQueries({ queryKey: COURT_LOAD_KEY })
...
export function useDeleteCourtWorkingHoursMonth(): UseMutationResult<
  void,
  Error,
...
> {
  ...
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ year, month }) => api.deleteCourtWorkingHoursMonth(year, month),
    onSuccess: () =>
      Promise.all([
        queryClient.invalidateQueries({ queryKey: COURT_WORKING_HOURS_KEY }),
        queryClient.invalidateQueries({ queryKey: COURT_LOAD_KEY })
...
export function useDeleteCourtWorkingHoursDay(): UseMutationResult<void, Error, string> {
  const api = useApiClient();
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (date) => api.deleteCourtWorkingHoursDay(date),
    onSuccess: () =>
      Promise.all([
        queryClient.invalidateQueries({ queryKey: COURT_WORKING_HOURS_KEY }),
        queryClient.invalidateQueries({ queryKey: COURT_LOAD_KEY })
