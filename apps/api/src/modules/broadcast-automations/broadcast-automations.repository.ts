...
export class BroadcastAutomationsRepository {
  ...
  async eventCoveredTrainingIdsSince(automationId: string, scheduledFor: Date, automationCreatedAt: Date, trainingIds: string[]): Promise<Set<string>> {
    if (!trainingIds.length) return new Set();
    ...
    const rows = await this.database.db.selectDistinct({ trainingId: tables.broadcastAutomationRunItemTrainings.trainingId }).from(tables.broadcastAutomationRunItemTrainings).innerJoin(tables.broadcastAutomationRuns, eq(tables.broadcastAutomationRunItemTrainings.runId, tables.broadcastAutomationRuns.id)).innerJoin(tables.broadcastAutomationDeliveries, eq(tables.broadcastAutomationDeliveries.runItemId, tables.broadcastAutomationRunItemTrainings.runItemId)).where(and(inArray(tables.broadcastAutomationRunItemTrainings.trainingId, trainingIds), inArray(tables.broadcastAutomationRuns.triggerKind, ["training-created", "training-time-changed", "freed-place"]), isNotNull(tables.broadcastAutomationRunItemTrainings.sourceEventId), eq(tables.broadcastAutomationDeliveries.outcome, "sent"), isNotNull(tables.broadcastAutomationDeliveries.completedAt), gt(tables.broadcastAutomationDeliveries.completedAt, since)));
    return new Set(rows.map((row) => row.trainingId));
  }
  ...
  async addTraining(runId: string, itemId: string, trainingId: string, snapshot: unknown, sourceEventId?: string, outcome: "included" | "skipped" = "included", skipReason: "training-covered-by-event" | undefined = undefined): Promise<void> { await this.database.db.insert(tables.broadcastAutomationRunItemTrainings).values({ runId, runItemId: itemId, trainingId, trainingSnapshot: snapshot, sourceEventId, outcome, skipReason }).onConflictDoNothing(); }
