...
function plan(overrides: Partial<MonthlySchedulePlan> = {}): MonthlySchedulePlan {
  return { id, startDate: "2026-08-01", endDate: "2026-08-28", timezone: "Europe/Belgrade", status: "draft", revision: 1, approvedRevision: null, generatedRevision: null, generatedAt: null, approvedAt: null, approvedBy: null, publishedAt: null, publishedBy: null, createdAt: "2026-01-01T00:00:00.000Z", updatedAt: "2026-01-01T00:00:00.000Z", templates: [], entries: [], ...overrides };
}

function fakeRepository(current = plan()) {
  const row = { id, startDate: current.startDate, endDate: current.endDate, status: current.status, revision: current.revision, approvedRevision: current.approvedRevision, generatedRevision: current.generatedRevision, generatedAt: current.generatedAt ? new Date(current.generatedAt) : null, approvedAt: current.approvedAt ? new Date(current.approvedAt) : null, approvedBy: current.approvedBy, publishedAt: null, publishedBy: null, createdBy: 7, updatedBy: 7, createdAt: new Date(current.createdAt), updatedAt: new Date(current.updatedAt) };
  return { transaction: vi.fn(async (work) => work({})), findPlanByPeriod: vi.fn(async () => row), lockPlannerRange: vi.fn(async () => undefined), listDaysOff: vi.fn(async () => []), overlaps: vi.fn(async () => ({ rows: [] })), createPlan: vi.fn(async () => row), lockPlan: vi.fn(async () => row), view: vi.fn(async () => current), findReference: vi.fn(async () => true), createTemplate: vi.fn(async () => ({ id:templateId, planId:id, groupId:id, daysOfWeek:[1,3], startTime:"18:00", endTime:"19:00", trainerId, preferredCourtId:null, createdAt:new Date(), updatedAt:new Date() })), rematerialize: vi.fn(async () => undefined), updatePlan: vi.fn(async (_id, patch) => Object.assign(row, patch)), findTemplate: vi.fn(async () => ({ id:templateId, planId:id, groupId:id, daysOfWeek:[1], startTime:"18:00", endTime:"19:00", trainerId, preferredCourtId:null, createdAt:new Date(), updatedAt:new Date() })), updateTemplate: vi.fn(async () => ({ id:templateId, planId:id, groupId:id, daysOfWeek:[1], startTime:"18:00", endTime:"19:00", trainerId, preferredCourtId:null, createdAt:new Date(), updatedAt:new Date() })), deleteTemplate: vi.fn(async () => undefined) } as unknown as MonthlyScheduleRepository;
}

describe("MonthlyScheduleService", () => {
  it("rejects non-admin callers before reads", async () => { const repo = fakeRepository(); const service = new MonthlyScheduleService(repo, adminEnv as never); await expect(service.get(99, { startDate:"2026-08-01", endDate:"2026-08-28" })).rejects.toBeInstanceOf(ForbiddenException); expect(repo.findPlanByPeriod).not.toHaveBeenCalled(); });
  it("gets an existing shared plan and create-or-get only inserts when absent", async () => { const repo = fakeRepository(); const service = new MonthlyScheduleService(repo, adminEnv as never); await service.createOrGet(7, { startDate:"2026-08-01", endDate:"2026-08-28" }); expect(repo.createPlan).not.toHaveBeenCalled(); vi.mocked(repo.findPlanByPeriod).mockResolvedValueOnce(undefined).mockResolvedValueOnce({ id, startDate:"2026-08-01", endDate:"2026-08-28" } as never); await service.createOrGet(7, { startDate:"2026-08-01", endDate:"2026-08-28" }); expect(repo.createPlan).toHaveBeenCalledWith("2026-08-01", "2026-08-28", 7, expect.anything()); });
  ...
  it("integrates current resource truth into read-only diagnostics and action flags", async () => {
    ...
    const result = await service.get(7, { startDate:"2026-08-01", endDate:"2026-08-28" });
